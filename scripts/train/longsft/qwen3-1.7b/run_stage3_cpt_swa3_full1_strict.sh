...
#
# By default, stage-2 DCP checkpoint is converted to HF format once and used as
# the student's initial weights (model_path). Set STAGE3_INIT_FROM=base to skip
# stage 2 and initialize the student directly from BASE_MODEL_PATH.
# load_checkpoint_path stays "auto" and only resumes stage-3's own checkpoints.
...
case "$STAGE3_INIT_FROM" in
  ...
  base)
    ...
    ;;
  *)
    echo "Unsupported STAGE3_INIT_FROM: $STAGE3_INIT_FROM" >&2
    echo "Use STAGE3_INIT_FROM=stage2 or STAGE3_INIT_FROM=base." >&2
    exit 1
    ;;
esac
